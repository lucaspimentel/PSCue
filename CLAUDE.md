...
8. **ReadyToRun for Module DLL**: `PSCue.Module.csproj` sets `PublishReadyToRun=true` to AOT-compile managed IL to native during publish, eliminating first-touch JIT on cold module import. Cascades to `PSCue.Shared.dll` and the SQLite chain. Requires `-r <RID>` on publish. `dotnet build` ignores R2R, so dev builds stay fast.
...
12. **PCD tab completion behavior**: CompletionText must match native cd exactly - use .\ prefix for child directories, ..\ for siblings, and single quotes for spaces. ListItemText should be clean (no prefixes/separators/quotes). See the `Smart Navigation (pcd)` region in module/Functions.ps1 for implementation.
...
14. **Release builds require `dotnet publish -r <RID>`**: The release workflow (.github/workflows/release.yml) and install-local.ps1 MUST use `dotnet publish` (not `dotnet build`) for PSCue.Module to include all dependencies. In addition, `PublishReadyToRun=true` in PSCue.Module.csproj requires `-r <RID>` on publish (R2R needs a target RID). With `-r`, the native SQLite library (`e_sqlite3.dll` / `libe_sqlite3.so`) is already flattened to the publish root -- no post-publish "flatten" step is needed. `dotnet build` is unaffected by `PublishReadyToRun` and remains fast for dev inner-loop work.
