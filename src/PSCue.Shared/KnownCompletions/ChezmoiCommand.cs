    ...
    public static Command Create()
    {
        return new Command("chezmoi")
        {
            ...
            [
                ...
                new("--config", "Set config file (-c)") { Alias = "-c" },
                new("--config-format", "Set config file format."),
                new("--debug", "Include debug information in output."),
                new("--destination", "Set destination directory (default ~) (-D)") { Alias = "-D" },
                new("--dry-run", "Do not make any modifications to the destination directory (-n)") { Alias = "-n" },
                new("--force", "Make all changes without prompting."),
                new("--help", "Help for chezmoi (-h)") { Alias = "-h" },
                new("--interactive", "Prompt for all changes."),
                new("--keep-going", "Keep going as far as possible after an error (-k)") { Alias = "-k" },
                new("--mode", "Mode (default file)."),
                new("--no-pager", "Do not use the pager."),
                new("--no-tty", "Do not attempt to get a TTY for prompts."),
                new("--output", "Write output to path instead of stdout (-o)") { Alias = "-o" },
                new("--persistent-state", "Set persistent state file."),
                new("--progress", "Display progress bars (default auto)."),
                new("--refresh-externals", "Refresh external cache (default auto) (-R)") { Alias = "-R" },
                new("--source", "Set source directory (default ~/.local/share/chezmoi) (-S)") { Alias = "-S" },
                ...
                new("--verbose", "Make output more verbose (-v)") { Alias = "-v" },
                new("--version", "Version for chezmoi."),
                new("--working-tree", "Set working tree directory (-W)") { Alias = "-W" },
            ]
        };
    }
}
